...
TEST(SerializationTest, CanSerializeConv2D) {
  ...
  Tensor b({.type = Type::kFP32, .shape = {1, 3, 3, 1}});
  Tensor c({.type = Type::kFP32, .shape = {1}});
  Tensor d = Conv2D(a, b, c, 2, 2, kPaddingSame, 1, 1,
  ...
  const auto* conv_options =
      reinterpret_cast<const TfLiteConvParams*>(
          node_and_reg->first.builtin_data);
...
TEST(SerializationTest, CanSerializeDepthwiseConv2D) {
  ...
  Tensor b({.type = Type::kFP32, .shape = {1, 3, 3, 1}});
  Tensor c({.type = Type::kFP32, .shape = {1}});
  Tensor d = DepthwiseConv2D(a, b, c, 2, 2, kPaddingSame, 1, 1, 1,
                             litert::tensor::FusedActivation::kActRelu);
  ASSERT_THAT(Save({d}, model_path), IsOk());
...
TEST(SerializationTest, CanSerializePad) {
  ...
  Tensor b({.type = Type::kI32, .shape = {4, 2}, .buffer = b_data});
  Tensor d = Pad(a, b);
  ASSERT_THAT(Save({d}, model_path), IsOk());
...
TEST(SerializationTest, CanSerializePadV2) {
  ...
  Tensor c({.type = Type::kFP32, .shape = {}});
  Tensor d = PadV2(a, b, c);
  ASSERT_THAT(Save({d}, model_path), IsOk());
...
TEST(SerializationTest, CanSerializeLogicalAnd) {
  ...
  Tensor a({.type = Type::kBOOL, .shape = {2, 5}});
  Tensor b({.type = Type::kBOOL, .shape = {2, 5}});
...
TEST(SerializationTest, CanSerializeMinimum) {
  ...
  Tensor a({.type = Type::kFP32, .shape = {2, 5}});
  Tensor b({.type = Type::kFP32, .shape = {2, 5}});
...
TEST(SerializationTest, CanSerializeSelectV2) {
  ...
  Tensor condition({.type = Type::kBOOL, .shape = {2, 5}});
  Tensor a({.type = Type::kFP32, .shape = {2, 5}});
  Tensor b({.type = Type::kFP32, .shape = {2, 5}});
