...
class ImageSegmentation:
  ...
  def _initialize(self) -> None:
    ...
    try:
      ...
      logging.info(
          "Model loaded successfully with %s signatures",
          self.model.get_num_signatures(),
      )
    except Exception as e:
      logging.error("Failed to create LiteRT model: %s", str(e))
      raise
  ...
  ) -> Tuple[np.ndarray, List[ColoredLabel]]:
...
def main():
  ...
  logging.info(
      "Segmentation finished in %.2f ms",
      inference_time,
  )

  # Create a colored mask
  colored_mask, _ = segmentation.create_colored_mask(mask)
  ...
  mask_path = os.path.join(output_dir, "%s_mask.png" % output_base)
  mask_image.save(mask_path)

  # Save blended image
  blended_image_pil = Image.fromarray(blended_image)
  blended_path = os.path.join(output_dir, "%s_blended.png" % output_base)
  ...
  logging.info("Mask: %s", mask_path)
  logging.info("Blended image: %s", blended_path)
