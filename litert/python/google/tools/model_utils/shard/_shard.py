...
from google3.third_party.odml.litert.litert.python.google.tools import model_utils as mu
from google3.third_party.odml.litert.litert.python.google.tools.model_utils.dialect import mlir
from google3.third_party.odml.litert.litert.python.google.tools.model_utils.dialect import tfl
import google3.third_party.odml.litert.litert.python.google.tools.model_utils.matcher as mm


def split(x: mlir.SSARankedTensorValue, dim: int, parts: int):
  mm.Match(isinstance(x.type, mlir.RankedTensorType))
  shape = x.type.shape
  if dim < 0:
    dim += len(shape)

  mm.Match(0 <= dim < len(shape))
  mm.Match(shape[dim] % parts == 0)
...
class BottomUpSharder:
  ...
  def _shard_broadcastable_binary(
    ...
    mm.Match(-len(o) <= dim < len(o))
    mm.Match(o[dim] % parts == 0)
  ...
  def _shard_conv2d(self, op: tfl.Conv2DOp, dim: int, parts: int):
    ...
    mm.Match(dim == len(shape) - 1)
    # If the output is used by another op, do not shard but add a split after.
    mm.Match(len(out.uses) == 1)
    mm.Match(out.type.shape[dim] % parts == 0)
