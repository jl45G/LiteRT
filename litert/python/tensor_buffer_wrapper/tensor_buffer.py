...
class TensorBuffer:
  ...
  This class provides a high-level interface to the underlying C++ LiteRtTensorBuffer
  implementation, allowing for creation, reading, writing, and management of tensor
  buffers in Python.
  """
  ...
  def create_from_host_memory(cls, data, dtype: str, num_elements: int):
    ...
    Args:
      data: Python object supporting the buffer protocol (e.g., numpy.ndarray, bytes).
