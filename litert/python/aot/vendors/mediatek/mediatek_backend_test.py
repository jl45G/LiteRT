...
from google3.third_party.odml.litert.litert.python.aot.core import components
from google3.third_party.odml.litert.litert.python.aot.core import test_common
from google3.third_party.odml.litert.litert.python.aot.core import types
from google3.third_party.odml.litert.litert.python.aot.vendors.mediatek import mediatek_backend
from google3.third_party.odml.litert.litert.python.aot.vendors.mediatek import target as target_lib
...
class MediaTekBackendTest(test_common.TestWithTfliteModels):

  @property
  def basic_config(self) -> types.Config:
    return {
        "backend_id": mediatek_backend.MediaTekBackend.id(),
  ...
  def test_unsupported_component(self):
    ...
    with self.assertRaisesRegex(
        NotImplementedError,
        "mediatek backend does not support mock_component component.",
    ):
  ...
  def test_bad_config(self):
    ...
    with self.assertRaisesRegex(ValueError, "Invalid backend id"):
      mediatek_backend.MediaTekBackend.create(config)

  def test_specialize_final(self):
    config = self.basic_config
    config["soc_model"] = target_lib.SocModel.MT6989.value
    backend = mediatek_backend.MediaTekBackend.create(config)
