...
void FillInputTensor(tflite::Interpreter& interpreter, float scale) {
  for (size_t k = 0; k < interpreter.inputs().size(); ++k) {
    ...
    if (tensor_ptr->type == kTfLiteInt16) {
      int16_t* p = interpreter.typed_input_tensor<int16_t>(k);
      for (int i = 0; i < tensor_elements_count; ++i) {
        p[i] = i % 2048 + 1 + k;
      }
    }
...
void PrintCpuNpuResultsDiff(tflite::Interpreter* cpu, tflite::Interpreter* npu,
  ...
  for (size_t i = 0; i < cpu->outputs().size(); ++i) {
    ...
    auto get_val = [&](tflite::Interpreter* interp, int output_index,
      ...
      if (tensor_ptr->type == kTfLiteInt16) {
        const int16_t* out = interp->typed_output_tensor<int16_t>(output_index);
        val = out[element_index];
      }
      if (tensor_ptr->type == kTfLiteInt8) {
...
TEST(InvokeModel, Veirify) {
  ...
  auto& npu_rt = **runtime;
  auto& interp = npu_rt.Interpreter();
...

  {
    ...
    auto build_tag_buf = GetMetadata(internal::kLiteRtBuildStampKey,
                                     *npu_rt.Flatbuffer().Unpack());
  ...
  LITERT_ASSIGN_OR_ABORT(
      auto options, CreateDispatchOptions(npu_rt.Flatbuffer().Buf().Data()));
  ...
  auto cpu_flatbuffer = FlatbufferWrapper::CreateFromTflFile(cpu_model_path);
  ABSL_CHECK(cpu_flatbuffer.HasValue()) << "Could not load cpu model";
  auto cpu_runtime =
      TflRuntime::CreateFromFlatBuffer(std::move(cpu_flatbuffer.Value()));
