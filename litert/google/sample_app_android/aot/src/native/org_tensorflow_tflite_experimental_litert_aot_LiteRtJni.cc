...
#include "litert/google/sample_app_android/aot/src/native/org_tensorflow_tflite_experimental_litert_aot_LiteRtJni.h"
...
void Java_org_tensorflow_tflite_experimental_litert_aot_LiteRtJni_runInference(
  ...
  ASSERT_EQ(&jni_helper, compilation_options.HasValue(), true);
  ASSERT_EQ(
      &jni_helper,
  ...
  auto res_compiled_model = CompiledModel::Create(
      *litert_env, *model, std::move(*compilation_options));
