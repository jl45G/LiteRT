...
# $3: SOC manufacturer (e.g. Qualcomm). Defaults to Qualcomm.
# $4: SOC model (e.g. V75). Defaults to V75.
# Output: $OUTPUT_MODEL
...
function apply_plugin_main {
  ...
  if (($# < 2)); then
    echo "Usage: $SCRIPT_NAME <apply|partition|compile|info|noop> <SOC manufacturer> <SOC model> <input model> <output model>"
    exit
  fi
  script_init
  apply_plugin apply $@
