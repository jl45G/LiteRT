 ...
 * This is required if LiteRT is built with LITERT_HOST_MEMORY_BUFFER_ALIGNMENT = 64.
 *
 * WARNING: Reflection on the "address" field can break on certain Android versions
 * or future devices that block reflection. You may need alternative methods.
 */
public final class AlignedBufferUtils {
  ...
  /**
   * Creates a 64-byte aligned ByteBuffer with a capacity of exactly {@code capacityBytes}.
   * The returned ByteBuffer:
   *   - is direct
   *   - ordered native
   *   - starts at a multiple-of-64 address
   */
  public static ByteBuffer create64ByteAlignedByteBuffer(int capacityBytes) {
    ...
    ByteBuffer big = ByteBuffer.allocateDirect(capacityBytes + (LITERT_ALIGNMENT - 1))
    ...
    if ((checkAddr % LITERT_ALIGNMENT) != 0) {
      throw new RuntimeException("Failed to achieve 64-byte alignment (address=0x"
          + Long.toHexString(checkAddr) + ")");
    }
    return aligned;
  }

  /**
   * Reflection-based helper to retrieve the native address of a direct ByteBuffer.
   * This may fail if hidden APIs or reflection are blocked on your Android platform.
   */
