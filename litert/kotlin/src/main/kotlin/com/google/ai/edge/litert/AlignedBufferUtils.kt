...
/**
 * Helper methods for creating ByteBuffers aligned to a 64-byte boundary without using reflection.
 * Instead, we call a JNI helper to retrieve the actual pointer.
 */
object AlignedBufferUtils {
  ...
  /**
   * Creates a 64-byte aligned ByteBuffer with capacity `capacityBytes`. If alignment fails or
   * reflection is blocked, you can handle fallback in your native code, or here.
   */
  @JvmStatic
  fun create64ByteAlignedByteBuffer(capacityBytes: Int): ByteBuffer {
    // Over-allocate by up to 63 bytes
    val big =
      ByteBuffer.allocateDirect(capacityBytes + LITERT_ALIGNMENT - 1).order(ByteOrder.nativeOrder())
    ...
    val alignedAddress =
      (baseAddress + (LITERT_ALIGNMENT - 1)) and ((LITERT_ALIGNMENT - 1).inv().toLong())
