...
#include "tensorflow/lite/experimental/litert/c/litert_accelerator.h"  // from @org_tensorflow
...
#include "tensorflow/lite/experimental/litert/cc/litert_environment.h"  // from @org_tensorflow
...
JNIEXPORT jlong JNICALL Java_com_google_ai_edge_litert_Environment_nativeCreate(
  ...
  if (num_tags > 0) {
    ...
    auto tags_array = env->GetIntArrayElements(tags, nullptr);
    for (int i = 0; i < num_tags; ++i) {
      jstring jstr =
          static_cast<jstring>(env->GetObjectArrayElement(values, i));
      ...
      auto option = Environment::Option{
          ...
          std::any(absl::string_view(value))};
      options.push_back(option);
      env->ReleaseStringUTFChars(jstr, value);
    }
    env->ReleaseIntArrayElements(tags, tags_array, JNI_ABORT);
  }
