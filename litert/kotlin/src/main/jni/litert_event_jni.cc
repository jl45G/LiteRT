...
#include "tensorflow/lite/experimental/litert/c/litert_event.h"  // from @org_tensorflow
#include "tensorflow/lite/experimental/litert/c/litert_event_type.h"  // from @org_tensorflow
...
JNIEXPORT void JNICALL Java_com_google_ai_edge_litert_Event_nativeWait(
  ...
  if (status != kLiteRtStatusOk) {
    LITERT_LOG(LITERT_ERROR, "EventWait failed, handle=%p, timeout=%lld ms",
               (void*)event_handle, (long long)timeout_ms);
  }
}
...
JNIEXPORT jint JNICALL Java_com_google_ai_edge_litert_Event_nativeGetType(
  ...
  if (!event_handle) return -1;
  LiteRtEventType t = kLiteRtEventTypeUnknown;
  LiteRtStatus status =
