    ...
    append_rule_kwargs(
        ...
        data = select({
            "@org_tensorflow//tensorflow:linux_x86_64": [
                "//third_party/neuro_pilot:v8_latest/host/lib/libneuron_adapter.so",
                "//third_party/neuro_pilot:v7_latest/host/lib/libneuron_adapter.so",
            ],
            "//conditions:default": [],
        }),
        linkopts = select({
            "@org_tensorflow//tensorflow:linux_x86_64": [
                make_rpaths(["//third_party/neuro_pilot:v8_latest/host/lib/libneuron_adapter.so"]),
                make_rpaths(["//third_party/neuro_pilot:v7_latest/host/lib/libneuron_adapter.so"]),
            ],
            "//conditions:default": [],
        }),
    )
